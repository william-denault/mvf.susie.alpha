...
## @param prior see multfsusie description
                                  ...
                                  )
{
  if(is.null(Y$Y_u)){
    ...
    if(missing(ind_analysis )){
      res_u   <- fsusieR:::cal_Bhat_Shat(Y$Y_u,X,sigma2 = 1)
    }else{
      res_u   <- fsusieR:::cal_Bhat_Shat(Y$Y_u,X,sigma2 = 1,lowc_wc=NULL,ind_analysis =ind_analysis$idx_u)
    }
