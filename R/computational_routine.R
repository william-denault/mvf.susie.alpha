...
#'
#' @param G_prior a scale specfic mash prior
#'
...
cal_lbf_mvfsusie_level <-  function(G_prior, tens_marg,s ,j , indx_lst)
{
  ...
  Bhat   <-   tens_marg$tens_Bhat[j,indx_lst[[s]],]
  Shat   <-   tens_marg$tens_Shat[j,indx_lst[[s]],]
  data   <-   mash_set_data( Bhat,  Shat)
  m      <-   G_prior[[s]]
...
log_BF_tens <- function( G_prior, tens_marg, indx_lst){
  ...
  lapply ( 1:length(indx_lst), function(s) cal_lbf_mvfsusie_level (G_prior, tens_marg,s ,j , indx_lst))
for ( j in 1:ncol(X))
{
...
l
    tens_marg$tens_Bhat[1,indx_lst[[s]][1],]
  ...
  lbf_scale  = dmvnorm(x =c(temp$Bhat) ,sigma = temp$S + U,log = TRUE) -
