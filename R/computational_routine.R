...
get_post_level <- function( G_prior, tens_marg, indx_lst,s, all=FALSE)
{
  ...
  if(!all)
  {
    PosteriorMean_level <- lapply( 1:dim(tens_marg$tens_Bhat)[1],
                                   function(j) res[[j]]$PosteriorMean
    )
    Posteriorsd_level   <- lapply( 1:dim(tens_marg$tens_Bhat)[1],
    ...
    lfdr_level          <- lapply( 1:dim(tens_marg$tens_Bhat)[1],
                                   function(j) res[[j]]$lfdr
    )
    NegativeProb_level  <- lapply( 1:dim(tens_marg$tens_Bhat)[1],
                                   function(j) res[[j]]$NegativeProb
    )
    lfsr_level          <- lapply( 1:dim(tens_marg$tens_Bhat)[1],
...
get_post_tens <- function(G_prior, tens_marg, indx_lst)
{

 tt <- lapply( 1: length(indx_lst),
               function(s)  get_post_level(G_prior, tens_marg, indx_lst,s)
               )
  simdata = simple_sims(50,5,1)
  data = mash_set_data(simdata$Bhat, simdata$Shat)
  m = mash(data, cov_canonical(data))
  mash_compute_posterior_matrices(m,data)$PosteriorSD
  str(mash_compute_posterior_matrices(m,data) )$PosteriorMean
